...
public class GameScene extends Scene {
	...
	private void initView() {
		...
		timerText = new Text(getCameraCenterX(), 10,
				FontRes.getFont(ConstantUtil.FONT_NAME_TIMER), "00",
				"00".length(), getVertexBufferObjectManager());
	...
	public void touchBlock(Block pBlock) {
		...
		if (gameStatus == ConstantUtil.GAME_START) {
			...
			if(moveNum == 0)
				moveDown(pBlock, 1);
			else if (pBlock.getRow() == moveNum + 2) {// 表示是在底部往上数的倒数第三行
	...
	private void moveDown(Block pBlock, int stepNum) {
		...
		// 移动步数加1
		moveNum++;
		timerText.setText(String.valueOf(moveNum));
		// 需要移动的距离
